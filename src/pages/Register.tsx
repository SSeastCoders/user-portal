...
export const Register: React.FC<RegisterProps> = ({}) => {
  ...
  const createUserMutation = useMutation(
    (user: FormValues) => axios.post(BASE_URL + '/users', user),
    {
  ...
  const loginMutation = useMutation(
    (loginInfo: LoginForm) => axios.post(BASE_URL + '/login', loginInfo),
    {
      onSuccess: ({headers}) => {
        console.log(headers);
  ...
  const isLoading = createUserMutation.isLoading || loginMutation.isLoading;
  ...
  const renderError = () => {
    if (createUserMutation.isError && errorRes.response !== undefined) {
      ...
      return (
        <div className="text-center invalid-feedback d-block">
          {loginErrRes.response.data.message}
        </div>
      );
    }
  };

  const checkboxInvalid = !('checked' in formErrors) ? '' : 'text-danger';
  return (
        ...
        <div className="card">
          <div className="card-body register-card-body">
            ...
            <div className="social-auth-links text-center">
              <p>- OR -</p>
              <ButtonSpinner block disabled={isLoading} icon="fab fa-facebook">
