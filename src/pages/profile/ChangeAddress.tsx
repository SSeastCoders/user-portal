...
const schema = Yup.object().shape({
  streetAddress: Yup.string()
    .max(50, "street address cant not be longer than 50 characters").required(),
  city: Yup.string()
    .max(50, "city cant not be longer than 50 characters").required(),
  state: Yup.string()
    .min(2)
    .max(2).required(),
...
export const ChangeAddress: React.FC<ChangeAddressProps> = ({ user }) => {
  ...
  const updateUser = useMutation((update: { address: Address }) => axiosToken.put(`${BASE_URL}/users/${state.id}`, update));
  const { formState: { errors }, handleSubmit, register, control, reset } = useForm<Address>({ defaultValues: user?.address, resolver: yupResolver(schema) });
